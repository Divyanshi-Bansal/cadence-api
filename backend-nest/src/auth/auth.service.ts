...
import { encryptDeterministic, hashToken } from '../lib/crypto';
...
export class AuthService {
  ...
  async sendMagicLink(email: string, name?: string): Promise<{ message: string; magicLink: string }> {
  ...
  async verifyMagicLink(rawToken: string): Promise<AuthResult> {
    ...
    const { accessToken, refreshToken } = await this.generateTokenPair(updatedUser.id);

    return { user: cleanUser, accessToken, refreshToken };
  }

  async signup(email: string, password?: string, name?: string, jobRole?: string): Promise<{ message: string; user: CleanUser; magicLink: string }> {
    const existing = await userRepository.findByEmail(email);

    if (existing) {
      const magicRes = await this.sendMagicLink(email, name);
      return {
        message: 'Account already exists! We have sent a magic link to your email to log you in.',
        user: existing,
    ...
    return {
      message: 'Account created successfully! We have sent a magic link to your email to log you in.',
      user,
      magicLink: magicRes.magicLink,
    };
  }
  ...
  async login(email: string, password?: string): Promise<AuthResult> {
    ...
    const isValidPassword = password ? await argon2.verify(rawUser.passwordHash, password) : false;
    if (!isValidPassword) {
      throw new AppError('Invalid email or password', 401);
    }

    const cleanUser = formatUser(rawUser);
    const { accessToken, refreshToken } = await this.generateTokenPair(rawUser.id);

    return { user: cleanUser, accessToken, refreshToken };
  }

  async getMe(userId: string): Promise<CleanUser> {
  ...
  async exchangeGithubCode(oneTimeCode: string): Promise<AuthResult> {
