...
// Stub checkCanInviteMember
const checkCanInviteMember = async (projectId: string, userId: string) => true;
...
5. Always be concise, professional, and clear.`;
    ...
    if (call.name === 'create_task') {
      ...
      const newStage = await prisma.boardStage.create({ data: { projectId, name: args.name, order: project.stages.length } });
      ...
      if (!targetStage) {
        ...
        const updated = await prisma.boardStage.update({ where: { id: targetStage.id }, data: { name: args.newStageName } });
      ...
      if (!targetStage) {
        ...
        if (tasksInStage.length > 0) {
          ...
          await prisma.boardStage.delete({ where: { id: targetStage.id } });
      ...
      try {
        const generated = []; // Mock generated
        actionResultText = `Successfully generated tickets.`;
        const createdTasks = [];
        ...
        actionResultText = `Generated and created ${createdTasks.length} tickets in ${project.stages[0].name}.`;
